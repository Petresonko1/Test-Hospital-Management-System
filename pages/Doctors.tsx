      ...
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-8 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            ...
            <input required placeholder="Dr. Jane Doe" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
          </div>
          ...
          <div className="flex space-x-3 pt-4">
            ...
            <button type="submit" className="flex-1 bg-blue-600 py-3 rounded-xl font-bold text-white hover:bg-blue-700 transition-colors">Add Staff</button>
          </div>
        </form>
      </div>
    </div>
...
const Doctors = () => {
  const { doctors, deleteDoctor } = useHospital();
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200"
        >
          + Add New Doctor
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {doctors.map((doctor) => (
          <div key={doctor.id} className="bg-white rounded-3xl border border-slate-200 p-6 flex flex-col items-center text-center hover:shadow-xl hover:shadow-slate-200 transition-all cursor-pointer group relative">
            <button 
